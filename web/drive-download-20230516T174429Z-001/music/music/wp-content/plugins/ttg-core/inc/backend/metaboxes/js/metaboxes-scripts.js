...
/* Igor: new uploader */
	...
	var ttgcore_waveresult = null;

	function ttgcore_filename(path){
	...
	/**
	 * [ttgCoreWaveformReload se ho salvato dati della wave, mostro wave]
	 * @since 2018 08 02
	 */
	function ttgCoreWaveformReload(){
		$('.qt-wafevorm-container').each(function(i,c){
			var t = $(this),
				tarea = t.find('textarea'),
				val = tarea.val();
			if( val !== ''){
				var canvasId = t.find('canvas').attr('id');
		...
		clndbtnimg.click(function() {
		    ...
		    file_frame = wp.media.frames.file_frame = wp.media({
     		...
     		var formID = $(this).attr('rel'),
				...
				/**
	         	 * @since 2018 08 02
	         	 */
				waveBlob = $(this).closest(".qw_repeatable_row").find(".qt-wafevorm-blob"),
				waveCanvas = $(this).closest(".qw_repeatable_row").find(".qt-wafevorm-canvas");
		    ...
		    file_frame.on( 'insert', function() {
		    	rawdata = file_frame.state().get( 'selection' );
		         ...
		         filename = ttgcore_filename(fileurl);
		         ...
		         if (qt_hasExtension(fileurl, ['.mp3'])) {
		         	 ...
		         	 */
					/*if( 'function' === typeof(ttgcore_generateWaveform) ){

						ttgcore_waveresult  = ttgcore_generateWaveform({
							audioId: id,
							audioUrl: fileurl,
							filename: filename,
							canvas: waveCanvas
						});
						var myCanvasContext = waveCanvas[0];
						var dataURL = myCanvasContext.toDataURL("image/png");
    					dataURL = dataURL.replace(/^data:image\/(png|jpeg);base64,/, "");   
						waveBlob.val( dataURL );
