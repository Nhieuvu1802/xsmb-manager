...
2020 08 12
* wp 5.5 update enqueued jquery-migrate

2018 08 02 
* Adding waveform generator (beta, not visible now)
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	$source = isset( $field['source'] ) ? $field['source'] : null;
	...
	switch( $type ) {
		...
		case 'waveform':
			...
			echo '
			<div class="qt-wafevorm-container">
			<textarea class="qt-wafevorm-blob" name="'.esc_attr( $name ).'" id="'.esc_attr( $id ).'" >'.esc_attr( $meta ).'</textarea>
			<canvas id="'.esc_attr( $name ).'_waveimg" class="qt-wafevorm-canvas" width="1800" height="280"></canvas>
			</div>';
...
class Custom_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			...
			/**
			 * @since  2018 08 02
			 * Ading waveform generator
			 */
			wp_enqueue_script( 'waveform', CUSTOM_METABOXES_DIR.'/js/waveform/modules/waveform.js', $deps, ttg_core_version() ); $deps[] = 'waveform';
			wp_enqueue_script( 'core-waveform', CUSTOM_METABOXES_DIR.'/js/waveform/core-waveform.js', $deps, ttg_core_version() ); $deps[] = 'core-waveform';
