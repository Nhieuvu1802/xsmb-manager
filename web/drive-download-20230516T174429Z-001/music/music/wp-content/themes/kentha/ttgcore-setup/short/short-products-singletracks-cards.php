 ...
 * =============================================
 */
if(!function_exists('kentha_single_tracks_cards_products_shortcode')){
	function kentha_single_tracks_cards_products_shortcode ($atts){
		...
		if ( $wp_query_singletracks->have_posts() ) : 
			?>
			<div class="qt-playlist-large qt-short-products-singletracks qt-woocommerce-singletrack-loop qt-woocommerce-singletrack-loop--small qt-woocommerce-singletrack-loop--cards">
				<ul class="qt-playlist row">
					<?php  
					while ( $wp_query_singletracks->have_posts() ) : $wp_query_singletracks->the_post();
						$post = $wp_query_singletracks->post;
						setup_postdata( $post );
						?><li class="qtmusicplayer-trackitem  qt-woocommerce-singletrack-loop__item qt-woocommerce-singletrack-loop__item--card qt-paper col s12 m<?php echo esc_attr( $colwidth_tablet); ?> l<?php echo esc_attr( $colwidth_desktop); ?>"><?php  
						get_template_part ('phpincludes/part-archive-item-product-singletrack-card', $list_design); 
						?></li><?php  
					endwhile;
					?>
				</ul>
...
if(function_exists('ttg_custom_shortcode')) {
	ttg_custom_shortcode("kentha-single-tracks-products-cards","kentha_single_tracks_cards_products_shortcode");
}
...
if(!function_exists('kentha_single_tracks_cards_products_shortcode_vc')){
	add_action( 'vc_before_init', 'kentha_single_tracks_cards_products_shortcode_vc' );
	function kentha_single_tracks_cards_products_shortcode_vc() {
  		vc_map( 

			array(
				 "name" => esc_html__( "Products single tracks cards", "kentha" ),
				 "base" => "kentha-single-tracks-products-cards",
