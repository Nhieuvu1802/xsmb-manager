...
function kentha_get_pluginslist( $url ){
	$plugins            = false;
	$current_theme      = wp_get_theme( );
	$theme_version      = $current_theme->get( 'Version' );
	$name               = $current_theme->get( 'Name' );
	$stored_optionname  = 'kentha_stored_required_plugins';
	...
	if( current_user_can('edit_others_pages') ) {
		...
		if( !$stored_plugins_list_json || $stored_plugins_list_json == ''){
			...
			$plugins_json = kentha_parse_plugins_update( $theme_version , $stored_optionname, $url );
		...
		if( $plugins_list ) {
			...
			return array();
			// add_action( 'admin_notices', 'kentha_plugins_notice__error' );
		}
