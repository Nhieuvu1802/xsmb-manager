 ...
 *  ==========================================================================
 *	VERY IMPORTANT MEMO:
 *	- If using WooCommerce, a similar copy of this file is in woocommerce-helpers/part-playlist-product.php
 *	- is loaded by woocommerce/single-product/meta.php
 *	Remember to bring there any change you do to this file
 * 	==========================================================================
 */
?>
...
<?php
$n = 1;
$events = get_post_meta(get_the_id(), 'track_repeatable');
if(array_key_exists(0, $events)){

	$events = $events[0];
	$thumb = get_the_post_thumbnail_url(null,'kentha-squared');
	...
	foreach($events as $track){ 
		...
		<li class="qtmusicplayer-trackitem">
			...
			/**
			 * =============================================================================
			 * Standard playable track
			 * =============================================================================
			 */
			$file = $track['releasetrack_mp3_demo'];
			if( $file != '' ){
				$regex_mp3 = "/.mp3/";
				if ( preg_match( $regex_mp3 , $file  ) ) {
					?><span class="qt-play qt-link-sec qtmusicplayer-play-btn" data-qtmplayer-cover="<?php echo esc_url($thumb); ?>" data-qtmplayer-file="<?php echo esc_url($file); ?>" data-qtmplayer-title="<?php echo esc_attr($track['releasetrack_track_title']); ?>" data-qtmplayer-artist="<?php echo esc_attr($track['releasetrack_artist_name']); ?>" data-qtmplayer-album="<?php echo esc_attr($title); ?>" data-qtmplayer-link="<?php echo esc_url($link); ?>" data-qtmplayer-buylink="<?php echo esc_attr($track['releasetrack_buyurl']); ?>" data-qtmplayer-price="<?php echo esc_url($track['price']); ?>" data-qtmplayer-icon="<?php echo esc_attr($icon); ?>" ><i class='material-icons'>play_circle_filled</i></span><?php 
				} else {
					?>
			...
			/**
			 * If you have a soundcloud link or youtube link, the player is added at the bottom
			 */
			if( $file != '' ){
				if ( !preg_match( $regex_mp3 , $file  ) ) {
					?><div class="qt-playlist__embed"><div data-autoembed="<?php echo esc_url( $file ); ?>"></div></div><?php
				}
			}
			?>
		</li>
					...
					<li class="qtmusicplayer-trackitem">
						...
						if($buyurl){ 
							...
							if(is_numeric($buylink)) {
								...
								$buylink = add_query_arg("add-to-cart" ,   $buylink, get_the_permalink());
								?>
								<a href="<?php echo esc_attr($buylink); ?>" data-quantity="1" data-product_id="<?php echo esc_attr($prodid); ?>" class="qt-cart product_type_simple add_to_cart_button ajax_add_to_cart"><i class="material-icons"><?php echo esc_html($icon); ?></i></a>
								<?php  
							} else {
								?>
								<a href="<?php echo esc_attr($buylink); ?>" class="qt-cart" target="_blank"><i class="material-icons"><?php echo esc_html($icon); ?></i></a>
								<?php
							}
						} ?>
					</li>
