...
CODE_BACKUP_DIR = _config.CODE_BACKUP_DIR
...
def install_update_zip(raw: bytes) -> tuple[str, Path]:
    ...
    allowed = {
        ...
        "sample_xsmb.csv", "version.txt", ".dockerignore", "Dockerfile", "Dockerfile.api",
        ...
        "xsmb_manager/api/routes.py", "xsmb_manager/api/main.py",
    }
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        ...
        timestamp = datetime.now(VN_TIMEZONE).strftime("%Y%m%d-%H%M%S")
        backup_dir = CODE_BACKUP_DIR / f"code-{timestamp}"
